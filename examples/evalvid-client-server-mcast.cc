...
//
  ...
  NS_LOG_INFO ("Create Multicast Routing Table");
//set up multicast routing for server --> mcast IPv4 address
  Ipv4StaticRoutingHelper multicast;
  Ptr<Node> sender = n.Get (1);
  Ptr<NetDevice> senderIf = d.Get (1);
  ...
  server.SetAttribute ("MutlicastGroupAddress",StringValue("225.1.2.4")); 
  ...
  client.SetAttribute ("MutlicastGroupAddress",StringValue("225.1.2.4")); 
  apps = client.Install (n.Get (0));
  apps.Start (Seconds (10.0));
  apps.Stop (Seconds (100.0));

  csma.EnablePcapAll ("evalvid-mcast", false);
